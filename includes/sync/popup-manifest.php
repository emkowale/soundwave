...
/* Starscream is the affiliate site's source of truth for popup state. */
function soundwave_popup_manifest_theme_state(): array {
  if (!function_exists('get_theme_mod') || !get_theme_mod('popup_store_enabled', false)) {
    return ['is_popup' => false];
  }

  $close = function_exists('starscream_get_popup_store_close_time')
    ? starscream_get_popup_store_close_time()
    : null;
  if (!$close instanceof DateTimeImmutable) return ['is_popup' => true];

  return [
    'is_popup' => true,
    'site_slug' => soundwave_popup_manifest_site_slug(),
    'closing_date' => wp_date('Y-m-d', $close->getTimestamp(), wp_timezone()),
    'closing_timestamp' => $close->getTimestamp(),
  ];
}

function soundwave_popup_manifest_defer_open_store_order(WC_Order $order): bool {
  $state = soundwave_popup_manifest_theme_state();
  if (empty($state['is_popup']) || empty($state['closing_timestamp']) || (int) $state['closing_timestamp'] <= time()) {
    return false;
  }

  // Save the active cycle while the storefront is open. The closed storefront
  // intentionally renders no banner, so this remains the close-time source.
  update_option('soundwave_popup_manifest_state', $state, false);
  return true;
}
...
function soundwave_popup_manifest_orders(array $state): array {
  ...
  if (!$close) return [];
  $orders = wc_get_orders(['limit' => -1, 'status' => ['processing'], 'orderby' => 'date', 'order' => 'ASC']);
  $eligible = [];
...
function soundwave_popup_manifest_run(): void {
  ...
  $master_id = absint($result['body']['master_order_id'] ?? 0);
  if (!empty($result['ok']) && !empty($result['body']['complete']) && $master_id > 0) {
    foreach ($orders as $order) {
      if (!$order instanceof WC_Order || $order->get_status() !== 'processing') continue;
      update_post_meta($order->get_id(), '_soundwave_popup_master_id', $master_id);
      $order->update_status('completed', 'Soundwave: popup cycle confirmed by Rumble work order #' . $master_id . '.');
    }
  }
