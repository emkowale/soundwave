...
function soundwave_settings_page(){
    ...
    if ( isset($_POST['soundwave_save']) && check_admin_referer('soundwave_save') ){
        ...
        $opts['popup_manifest_secret'] = sanitize_text_field( $_POST['popup_manifest_secret'] ?? '' );
    ...
    echo '<tr><th><label for="popup_manifest_secret">Popup Manifest Secret</label></th><td><input type="password" class="regular-text code" id="popup_manifest_secret" name="popup_manifest_secret" value="'.esc_attr($opts['popup_manifest_secret'] ?? '').'" autocomplete="new-password"><p class="description">Must match Rumble. It signs the final popup-store order manifest.</p></td></tr>';
    echo '</table>';
