 ...
 * Version: 1.1.30
 * Last Updated: 2025-09-18 14:30 EDT
 */
...
if ( ! function_exists('sw_placeholder_product_id') ) {
  function sw_placeholder_product_id(): int { return 40158; }
}
...
if ( ! function_exists('sw_add_affiliate_item') ) {
  function sw_add_affiliate_item(WC_Order $order, array $src): void {
    ...
    if ( ! empty($src['name']) )  { $item->add_meta_data('_sw_source_name',  (string) $src['name']); }
    if ( ! empty($src['sku']) )   { $item->add_meta_data('_sw_source_sku',   (string) $src['sku']); }
    if ( ! empty($src['image']) ) { $item->add_meta_data('_sw_source_image', (string) $src['image']); }
...
if ( ! function_exists('sw_finalize_order_for_analytics') ) {
  function sw_finalize_order_for_analytics(WC_Order $order, array $ctx = []): void {
    ...
    $order->save();
    $order->update_status('processing', 'Created by Soundwave import.');
