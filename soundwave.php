...
Requires PHP: 7.2
Requires Plugins: woocommerce, bumblebee
Author: Eric Kowalewski
License: GPL2
*/

if ( ! defined('ABSPATH') ) exit;
define('SOUNDWAVE_VERSION','1.4.42');
define('SOUNDWAVE_DIR', plugin_dir_path(__FILE__));
define('SOUNDWAVE_URL', plugin_dir_url(__FILE__));

register_deactivation_hook(__FILE__, function(){
    if (function_exists('wp_clear_scheduled_hook')) {
        wp_clear_scheduled_hook('soundwave_resync_unsynced_batch');
        wp_clear_scheduled_hook('soundwave_popup_manifest_check');
    }
});
...
add_action('plugins_loaded', function(){
    ...
    if ( ! empty($missing) ) {
        ...
        add_action('admin_notices', function() use ($missing){
            ...
            echo '<div class="notice notice-error"><p><strong>Soundwave requires: ' . esc_html($required) . '.</strong> The plugin was deactivated.</p></div>';
        });
        return;
    }
